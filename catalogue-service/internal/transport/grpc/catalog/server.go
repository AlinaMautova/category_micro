...
import (
	...
	"catalogue-service/internal/lib/models"
	"context"
	"errors"
	"fmt"
	"github.com/jinzhu/copier"
	cataloguep "github.com/lyerzhanl/contracts/gen/go/catalog"
...
func (cs *catalogueService) CreateItem(ctx context.Context, req *cataloguep.CreateItemRequest) (*cataloguep.CreateItemResponse, error) {
	...
	if req.Item.PhotoUrl == "" {
		return nil, status.Error(codes.InvalidArgument, "photo url is required")
	}
	...
	if err != nil {
		switch {
		case errors.Is(err, lib.ErrItemAlreadyExist):
